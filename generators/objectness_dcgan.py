#dcgan but with an addition that the generator
# maximizes the objectness according to a pre-trained classifier
...
if __name__ == '__main__':
    ...
    for epoch in range(opt.niter):
        for i, data in enumerate(dataloader):
            ...
            objectness = pr[:, 0].mean()
            output = netD(fake)
            errG = criterion(output, label) - objectness
            errG.backward()
            D_G_z2 = output.data.mean()
            optimizerG.step()

            print('[%d/%d][%d/%d] OBJ:%.4f Loss_D: %.4f Loss_G: %.4f D(x): %.4f D(G(z)): %.4f / %.4f'
